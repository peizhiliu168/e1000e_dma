...
static uintptr_t virt_to_phys(void *virt) {
  ...
  ASSERT(entry != 0,
         "failed to get physical address for %p (perhaps forgot sudo?)", virt);
  close(fd);
...
static void open_vfio(struct device *dev, int segn, int busn, int devn,
  ...
  snprintf(path, sizeof(path), "%04x:%02x:%02x.%01x", segn, busn, devn, funcn);
...
static void get_device_info(struct device *dev) {
  ...
  for (i = 0; i < dev->device_info.num_regions; i++) {
    ...
    debug("region %d.flags = %d%d%d%d (%s)", i,
          ...
          !!(dev->regs[i].flags & VFIO_REGION_INFO_FLAG_READ), region_name[i]);
  }
...
void init_device(struct device *dev) {
  ...
  debug("RDBAL: %08X, RDBAH=%08X", read_u32(dev, RDBAL), read_u32(dev, RDBAH));
  debug("RDLEN: %08X", read_u32(dev, RDLEN));
  debug("TCTL:  %08X", read_u32(dev, TCTL));
  debug("TDBAL: %08X, TDBAH=%08X", read_u32(dev, TDBAL), read_u32(dev, TDBAH));
...
void dump_pkt(void *addr) {
  ...
  printf("src=%02X:%02X:%02X:%02X:%02X:%02X\n", eth->h_source[5],
         eth->h_source[4], eth->h_source[3], eth->h_source[2], eth->h_source[1],
         eth->h_source[0]);

  printf("dst=%02X:%02X:%02X:%02X:%02X:%02X\n", eth->h_dest[5], eth->h_dest[4],
         eth->h_dest[3], eth->h_dest[2], eth->h_dest[1], eth->h_dest[0]);
  printf("proto=%04X\n", ntohs(eth->h_proto));
}
...
static ssize_t create_dummy_packet(struct device *dev, char *buf) {
  ...
  udp->uh_sum = 0;
  buf[sizeof(struct ethhdr) + sizeof(struct ip) + sizeof(struct udphdr)] = 'a';

  return sizeof(struct ethhdr) + sizeof(struct ip) + sizeof(struct udphdr) + 1;
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2 ||
      sscanf(argv[1], "%04x:%02x:%02x.%d", &segn, &busn, &devn, &funcn) != 4) {

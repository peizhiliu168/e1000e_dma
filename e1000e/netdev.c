  ...
  for (i = 0; i < adapter->rx_ps_pages; i++) {
    ...
    if (ps_page->page) {
      ...
      print_hex_dump(KERN_INFO, "", DUMP_PREFIX_ADDRESS, 16, 1,
                     page_address(ps_page->page), PAGE_SIZE, true);
    }
  }
}
...
static void e1000e_dump(struct e1000_adapter *adapter) {
  ...
  for (reginfo = (struct e1000_reg_info *)e1000_reg_info_tbl; reginfo->name;
       reginfo++) {
  ...
  pr_info(" %5d %5X %5X %016llX %04X %3X %016llX\n", 0, tx_ring->next_to_use,
          tx_ring->next_to_clean, (unsigned long long)buffer_info->dma,
   ...
   *   63       48 47     40 39  36 35    32 31     24 23  20 19        0
   */
  pr_info("Tl[desc]     [address 63:0  ] [SpeCssSCmCsLen] [bi->dma       ] "
  ...
  for (i = 0; tx_ring->desc && (i < tx_ring->count); i++) {
    ...
    pr_info("T%c[0x%03X]    %016llX %016llX %016llX %04X  %3X %016llX %p%s\n",
            (!(le64_to_cpu(u0->b) & BIT(29))
                 ? 'l'
                 : ((le64_to_cpu(u0->b) & BIT(20)) ? 'd' : 'c')),
            i, (unsigned long long)le64_to_cpu(u0->a),
            (unsigned long long)le64_to_cpu(u0->b),
            (unsigned long long)buffer_info->dma, buffer_info->length,
            buffer_info->next_to_watch,
            (unsigned long long)buffer_info->time_stamp, buffer_info->skb,
            next_desc);

    if (netif_msg_pktdata(adapter) && buffer_info->skb)
      print_hex_dump(KERN_INFO, "", DUMP_PREFIX_ADDRESS, 16, 1,
                     buffer_info->skb->data, buffer_info->skb->len, true);
  }
...
rx_ring_summary:
  ...
  case 3:
     ...
     *    +-----------------------------------------------------+
     */
    pr_info("R  [desc]      [buffer 0 63:0 ] [buffer 1 63:0 ] [buffer 2 63:0 ] "
            "[buffer 3 63:0 ] [bi->dma       ] [bi->skb] <-- Ext Pkt Split "
            "format\n");
     ...
     */
    pr_info("RWB[desc]      [ck ipid mrqhsh] [vl   l0 ee  es] [ l3  l2  l1 hs] "
            "[reserved      ] ---------------- [bi->skb] <-- Ext Rx Write-Back "
            "format\n");
    for (i = 0; i < rx_ring->count; i++) {
      ...
      if (staterr & E1000_RXD_STAT_DD) {
        /* Descriptor Done */
        pr_info("%s[0x%03X]     %016llX %016llX %016llX %016llX "
                "---------------- %p%s\n",
                "RWB", i, (unsigned long long)le64_to_cpu(u1->a),
                (unsigned long long)le64_to_cpu(u1->b),
                (unsigned long long)le64_to_cpu(u1->c),
                (unsigned long long)le64_to_cpu(u1->d), buffer_info->skb,
                next_desc);
      } else {
        pr_info("%s[0x%03X]     %016llX %016llX %016llX %016llX %016llX %p%s\n",
                "R  ", i, (unsigned long long)le64_to_cpu(u1->a),
                (unsigned long long)le64_to_cpu(u1->b),
                (unsigned long long)le64_to_cpu(u1->c),
                (unsigned long long)le64_to_cpu(u1->d),
                (unsigned long long)buffer_info->dma, buffer_info->skb,
                next_desc);
  ...
  case 0:
     ...
     *   +-----------------------------------------------------+
     */
    pr_info("R  [desc]      [buf addr 63:0 ] [reserved 63:0 ] [bi->dma       ] "
     ...
     *   63       48 47    32 31            20 19               0
     */
    pr_info("RWB[desc]      [cs ipid    mrq] [vt   ln xe  xs] [bi->skb] <-- "
    ...
    for (i = 0; i < rx_ring->count; i++) {
      ...
      if (staterr & E1000_RXD_STAT_DD) {
        ...
        pr_info("%s[0x%03X]     %016llX %016llX ---------------- %p%s\n", "RWB",
                i, (unsigned long long)le64_to_cpu(u1->a),
                (unsigned long long)le64_to_cpu(u1->b), buffer_info->skb,
                next_desc);
      } else {
        ...
        if (netif_msg_pktdata(adapter) && buffer_info->skb)
          print_hex_dump(KERN_INFO, "", DUMP_PREFIX_ADDRESS, 16, 1,
                         buffer_info->skb->data, adapter->rx_buffer_len, true);
      }
    }
  }
}
  ...
  while (cleaned_count--) {
    ...
    for (j = 0; j < PS_PAGE_BUFFERS; j++) {
      ...
      if (!ps_page->page) {
        ...
        ps_page->dma = dma_map_page(&pdev->dev, ps_page->page, 0, PAGE_SIZE,
                                    DMA_FROM_DEVICE);
        if (dma_mapping_error(&pdev->dev, ps_page->dma)) {
    ...
    skb = __netdev_alloc_skb_ip_align(netdev, adapter->rx_ps_bsize0, gfp);
...
static void e1000_alloc_jumbo_rx_buffers(struct e1000_ring *rx_ring,
  ...
  check_page:
    ...
    if (!buffer_info->dma) {
      buffer_info->dma = dma_map_page(&pdev->dev, buffer_info->page, 0,
...
static void e1000_put_txbuf(struct e1000_ring *tx_ring,
                            struct e1000_buffer *buffer_info, bool drop) {
  struct e1000_adapter *adapter = tx_ring->adapter;

  if (buffer_info->dma) {
    if (buffer_info->mapped_as_page)
      dma_unmap_page(&adapter->pdev->dev, buffer_info->dma, buffer_info->length,
                     DMA_TO_DEVICE);
    else
...
static void e1000_print_hw_hang(struct work_struct *work) {
  struct e1000_adapter *adapter =
      container_of(work, struct e1000_adapter, print_hang_task);
  struct net_device *netdev = adapter->netdev;
  struct e1000_ring *tx_ring = adapter->tx_ring;
  ...
  e_err("Detected Hardware Unit Hang:\n"
        ...
        tx_ring->next_to_clean, tx_ring->buffer_info[eop].time_stamp, eop,
        jiffies, eop_desc->upper.fields.status, er32(STATUS), phy_status,
        phy_1000t_status, phy_ext_status, pci_status);
...
static void e1000e_tx_hwtstamp_work(struct work_struct *work) {
  struct e1000_adapter *adapter =
      container_of(work, struct e1000_adapter, tx_hwtstamp_work);
  struct e1000_hw *hw = &adapter->hw;
...
static bool e1000_clean_rx_irq_ps(struct e1000_ring *rx_ring, int *work_done,
  ...
  while (staterr & E1000_RXD_STAT_DD) {
    ...
    cleaned = true;
    cleaned_count++;
    dma_unmap_single(&pdev->dev, buffer_info->dma, adapter->rx_ps_bsize0,
                     DMA_FROM_DEVICE);
    buffer_info->dma = 0;
...

    {
      ...
      if (l1 && (l1 <= copybreak) && ((length + l1) <= adapter->rx_ps_bsize0)) {
        ...
        dma_sync_single_for_cpu(&pdev->dev, ps_page->dma, PAGE_SIZE,
                                DMA_FROM_DEVICE);
        memcpy(skb_tail_pointer(skb), page_address(ps_page->page), l1);
        dma_sync_single_for_device(&pdev->dev, ps_page->dma, PAGE_SIZE,
                                   DMA_FROM_DEVICE);
...
static bool e1000_clean_jumbo_rx_irq(struct e1000_ring *rx_ring, int *work_done,
  ...
  while (staterr & E1000_RXD_STAT_DD) {
    ...
    cleaned = true;
    cleaned_count++;
    dma_unmap_page(&pdev->dev, buffer_info->dma, PAGE_SIZE, DMA_FROM_DEVICE);
    buffer_info->dma = 0;
...
#define rxtop (rx_ring->rx_skb_top)
    if (!(staterr & E1000_RXD_STAT_EOP)) {
      ...
      if (!rxtop) {
        ...
        skb_fill_page_desc(rxtop, shinfo->nr_frags, buffer_info->page, 0,
                           length);
      ...
      if (rxtop) {
        ...
        if (length <= copybreak && skb_tailroom(skb) >= length) {
          memcpy(skb_tail_pointer(skb), page_address(buffer_info->page),
                 length);
...
static void e1000e_downshift_workaround(struct work_struct *work) {
  struct e1000_adapter *adapter =
      container_of(work, struct e1000_adapter, downshift_task);
...
static irqreturn_t e1000_intr_msi(int __always_unused irq, void *data) {
  ...
  if ((icr & E1000_ICR_ECCER) && (hw->mac.type >= e1000_pch_lpt)) {
    ...
    adapter->corr_errors += pbeccsts & E1000_PBECCSTS_CORR_ERR_CNT_MASK;
...
static void e1000_configure_msix(struct e1000_adapter *adapter) {
  ...
  if (rx_ring->itr_val)
    writel(1000000000 / (rx_ring->itr_val * 256), rx_ring->itr_register);
  else
  ...
  vector++;
  if (tx_ring->itr_val)
    writel(1000000000 / (tx_ring->itr_val * 256), tx_ring->itr_register);
  else
  ...
  case E1000E_INT_MODE_MSIX:
    if (adapter->flags & FLAG_HAS_MSIX) {
      ...
      adapter->msix_entries =
          kcalloc(adapter->num_vectors, sizeof(struct msix_entry), GFP_KERNEL);
      if (adapter->msix_entries) {
        ...
        err = pci_enable_msix_range(a->pdev, a->msix_entries, a->num_vectors,
                                    a->num_vectors);
        if (err > 0)
          return;
      }
...
static int e1000_request_msix(struct e1000_adapter *adapter) {
  ...
  if (strlen(netdev->name) < (IFNAMSIZ - 5))
    snprintf(adapter->rx_ring->name, sizeof(adapter->rx_ring->name) - 1,
             "%.14s-rx-0", netdev->name);
  else
    memcpy(adapter->rx_ring->name, netdev->name, IFNAMSIZ);
  err = request_irq(adapter->msix_entries[vector].vector, e1000_intr_msix_rx, 0,
                    adapter->rx_ring->name, netdev);
  if (err)
    return err;
  adapter->rx_ring->itr_register =
      adapter->hw.hw_addr + E1000_EITR_82574(vector);
  adapter->rx_ring->itr_val = adapter->itr;
  vector++;

  if (strlen(netdev->name) < (IFNAMSIZ - 5))
    snprintf(adapter->tx_ring->name, sizeof(adapter->tx_ring->name) - 1,
             "%.14s-tx-0", netdev->name);
  else
    memcpy(adapter->tx_ring->name, netdev->name, IFNAMSIZ);
  err = request_irq(adapter->msix_entries[vector].vector, e1000_intr_msix_tx, 0,
                    adapter->tx_ring->name, netdev);
  if (err)
    return err;
  adapter->tx_ring->itr_register =
      adapter->hw.hw_addr + E1000_EITR_82574(vector);
  adapter->tx_ring->itr_val = adapter->itr;
  vector++;

  err = request_irq(adapter->msix_entries[vector].vector, e1000_msix_other, 0,
                    netdev->name, netdev);
  if (err)
    return err;
...
static int e1000_request_irq(struct e1000_adapter *adapter) {
  ...
  if (adapter->flags & FLAG_MSI_ENABLED) {
    err = request_irq(adapter->pdev->irq, e1000_intr_msi, 0, netdev->name,
                      netdev);
    if (!err)
      return err;
  ...
  err = request_irq(adapter->pdev->irq, e1000_intr, IRQF_SHARED, netdev->name,
                    netdev);
  if (err)
...
static int e1000_alloc_ring_dma(struct e1000_adapter *adapter,
  ...
  ring->desc =
      dma_alloc_coherent(&pdev->dev, ring->size, &ring->dma, GFP_KERNEL);
  if (!ring->desc)
    return -ENOMEM;
...
int e1000e_setup_rx_resources(struct e1000_ring *rx_ring) {
  ...
  for (i = 0; i < rx_ring->count; i++) {
    ...
    buffer_info->ps_pages =
        kcalloc(PS_PAGE_BUFFERS, sizeof(struct e1000_ps_page), GFP_KERNEL);
...
static void e1000_set_itr(struct e1000_adapter *adapter) {
  ...
  adapter->tx_itr = e1000_update_itr(adapter->tx_itr, adapter->total_tx_packets,
                                     adapter->total_tx_bytes);
  /* conservative mode (itr 3) eliminates the lowest_latency setting */
  if (adapter->itr_setting == 3 && adapter->tx_itr == lowest_latency)
    adapter->tx_itr = low_latency;

  adapter->rx_itr = e1000_update_itr(adapter->rx_itr, adapter->total_rx_packets,
...
static void e1000_init_manageability_pt(struct e1000_adapter *adapter) {
  ...
  case e1000_82583:
    ...
    for (i = 0, j = 0; i < 8; i++)
      if (er32(MDEF(i)) == 0) {
        ew32(MDEF(i), (E1000_MDEF_PORT_623 | E1000_MDEF_PORT_664));
        manc2h |= BIT(1);
        j++;
        break;
      }
...
static void e1000_configure_tx(struct e1000_adapter *adapter) {
  ...
  if (adapter->flags2 & FLAG2_DMA_BURST) {
    ...
    txdctl &=
        ~(E1000_TXDCTL_PTHRESH | E1000_TXDCTL_HTHRESH | E1000_TXDCTL_WTHRESH);
...
#define PAGE_USE_COUNT(S)                                                      \
  (((S) >> PAGE_SHIFT) + (((S) & (PAGE_SIZE - 1)) ? 1 : 0))
...
static void e1000_configure_rx(struct e1000_adapter *adapter) {
  ...
  if (adapter->netdev->mtu > ETH_DATA_LEN) {
    u32 lat =
        ((er32(PBA) & E1000_PBA_RXA_MASK) * 1024 - adapter->max_frame_size) *
        8 / 1000;
    ...
    dev_info(&adapter->pdev->dev, "Some CPU C-states have been disabled in "
                                  "order to enable jumbo frames\n");
    cpu_latency_qos_update_request(&adapter->pm_qos_req, lat);
  } else {
    cpu_latency_qos_update_request(&adapter->pm_qos_req, PM_QOS_DEFAULT_VALUE);
  }
...
static int e1000e_config_hwtstamp(struct e1000_adapter *adapter,
  ...
  if ((er32(TSYNCRXCTL) &
       (E1000_TSYNCRXCTL_ENABLED | E1000_TSYNCRXCTL_TYPE_MASK)) !=
      (regval & (E1000_TSYNCRXCTL_ENABLED | E1000_TSYNCRXCTL_TYPE_MASK))) {
...
static void e1000_flush_desc_rings(struct e1000_adapter *adapter) {
  ...
  tdlen = er32(TDLEN(0));
  pci_read_config_word(adapter->pdev, PCICFG_DESC_RING_STATUS, &hang_state);
...
static void e1000e_systim_reset(struct e1000_adapter *adapter) {
  ...
  spin_lock_irqsave(&adapter->systim_lock, flags);
  timecounter_init(&adapter->tc, &adapter->cc, ktime_to_ns(ktime_get_real()));
  spin_unlock_irqrestore(&adapter->systim_lock, flags);
...
void e1000e_reset(struct e1000_adapter *adapter) {
  ...
  if (adapter->max_frame_size > (VLAN_ETH_FRAME_LEN + ETH_FCS_LEN)) {
     ...
     */
    min_tx_space =
        (adapter->max_frame_size + sizeof(struct e1000_tx_desc) - ETH_FCS_LEN) *
        2;
   ...
   */
  adapter->tx_fifo_limit = min_t(u32, ((er32(PBA) >> 16) << 10) - 96, 24 << 10);
  ...
  if (adapter->flags2 & FLAG2_HAS_EEE) {
    ...
    e1000_write_emi_reg_locked(
        hw, adv_addr,
        hw->dev_spec.ich8lan.eee_disable ? 0 : adapter->eee_advert);
  ...
  if (hw->mac.type >= e1000_pch_spt && adapter->int_mode == 0) {
    ...
    reg = er32(FEXTNVM9);
    reg |= E1000_FEXTNVM9_IOSFSB_CLKGATE_DIS | E1000_FEXTNVM9_IOSFSB_CLKREQ_DIS;
    ew32(FEXTNVM9, reg);
  }
}
...
static u64 e1000e_cyclecounter_read(struct cyclecounter *cc) {
  struct e1000_adapter *adapter = container_of(cc, struct e1000_adapter, cc);
...
static int e1000_test_msi_interrupt(struct e1000_adapter *adapter) {
  ...
  err = request_irq(adapter->pdev->irq, e1000_intr_msi_test, 0, netdev->name,
                    netdev);
  if (err) {
    pci_disable_msi(adapter->pdev);
    goto msi_test_failed;
...
static void e1000e_update_phy_task(struct work_struct *work) {
  struct e1000_adapter *adapter =
      container_of(work, struct e1000_adapter, update_phy_task);
  struct e1000_hw *hw = &adapter->hw;
...
static void e1000e_update_phy_stats(struct e1000_adapter *adapter) {
   ...
   */
  hw->phy.addr = 1;
  ret_val = e1000e_read_phy_reg_mdic(hw, IGP01E1000_PHY_PAGE_SELECT, &phy_data);
  if (ret_val)
    goto release;
  if (phy_data != (HV_STATS_PAGE << IGP_PAGE_SHIFT)) {
    ret_val = hw->phy.ops.set_page(hw, HV_STATS_PAGE << IGP_PAGE_SHIFT);
    if (ret_val)
      goto release;
  }
...
static void e1000e_update_stats(struct e1000_adapter *adapter) {
   ...
   */
  netdev->stats.rx_errors = adapter->stats.rxerrc + adapter->stats.crcerrs +
                            adapter->stats.algnerrc + adapter->stats.ruc +
                            adapter->stats.roc + adapter->stats.cexterr;
...
static void e1000_phy_read_status(struct e1000_adapter *adapter) {
  ...
  if (!pm_runtime_suspended((&adapter->pdev->dev)->parent) &&
    ...
    phy->bmsr = (BMSR_100FULL | BMSR_100HALF | BMSR_10FULL | BMSR_10HALF |
                 BMSR_ESTATEN | BMSR_ANEGCAPABLE | BMSR_ERCAP);
...
static void e1000_watchdog_task(struct work_struct *work) {
  ...
  if (link) {
    if (!netif_carrier_ok(netdev)) {
      ...
      mac->ops.get_link_up_info(&adapter->hw, &adapter->link_speed,
                                &adapter->link_duplex);
...
link_up:
  ...
  if (adapter->itr_setting == 4) {
    ...
    u32 dif = (adapter->gotc > adapter->gorc ? adapter->gotc - adapter->gorc
                                             : adapter->gorc - adapter->gotc) /
              10000;
...
static int e1000_tso(struct e1000_ring *tx_ring, struct sk_buff *skb,
  ...
  if (protocol == htons(ETH_P_IP)) {
    ...
    iph->check = 0;
    tcp_hdr(skb)->check =
        ~csum_tcpudp_magic(iph->saddr, iph->daddr, 0, IPPROTO_TCP, 0);
  ...
  cmd_length |= (E1000_TXD_CMD_DEXT | E1000_TXD_CMD_TSE | E1000_TXD_CMD_TCP |
                 (skb->len - (hdr_len)));
...
static int e1000_tx_map(struct e1000_ring *tx_ring, struct sk_buff *skb,
  ...
  while (len) {
    ...
    buffer_info->next_to_watch = i;
    buffer_info->dma =
        dma_map_single(&pdev->dev, skb->data + offset, size, DMA_TO_DEVICE);
  ...
  for (f = 0; f < nr_frags; f++) {
    ...
    while (len) {
      ...
      buffer_info->next_to_watch = i;
      buffer_info->dma =
          skb_frag_dma_map(&pdev->dev, frag, offset, size, DMA_TO_DEVICE);
...
static netdev_tx_t e1000_xmit_frame(struct sk_buff *skb,
  ...
  if (count) {
    ...
    e1000_maybe_stop_tx(
        tx_ring,
        ((MAX_SKB_FRAGS + 1) * DIV_ROUND_UP(PAGE_SIZE, adapter->tx_fifo_limit) +
         4));
...
void e1000e_get_stats64(struct net_device *netdev,
   ...
   * our own version based on RUC and ROC
   */
  stats->rx_errors = adapter->stats.rxerrc + adapter->stats.crcerrs +
                     adapter->stats.algnerrc + adapter->stats.ruc +
                     adapter->stats.roc + adapter->stats.cexterr;
...
static int e1000_change_mtu(struct net_device *netdev, int new_mtu) {
  ...
  if ((adapter->hw.mac.type >= e1000_pch2lan) &&
      !(adapter->flags2 & FLAG2_CRC_STRIPPING) && (new_mtu > ETH_DATA_LEN)) {
...
static int e1000_init_phy_wakeup(struct e1000_adapter *adapter, u32 wufc) {
  ...
  for (i = 0; i < adapter->hw.mac.mta_reg_count; i++) {
    ...
    hw->phy.ops.write_reg_page(hw, BM_MTA(i), (u16)(mac_reg & 0xFFFF));
  ...
  if (mac_reg & E1000_RCTL_MO_3)
    phy_reg |= (FIELD_GET(E1000_RCTL_MO_3, mac_reg) << BM_RCTL_MO_SHIFT);
...
static void e1000e_s0ix_entry_flow(struct e1000_adapter *adapter) {
  ...
  if (er32(FWSM) & E1000_ICH_FWSM_FW_VALID && hw->mac.type >= e1000_pch_adp) {
...
static int __e1000_shutdown(struct pci_dev *pdev, bool runtime) {
   ...
   * for EEE in Sx
   */
  if ((hw->phy.type >= e1000_phy_i217) && adapter->eee_advert &&
    ...
    if (!retval) {
      ...
      if (!retval) {
        if (adapter->eee_advert & hw->dev_spec.ich8lan.eee_lp_ability &
        ...
        retval = e1e_wphy_locked(hw, I82579_LPI_CTRL, lpi_ctrl);
      }
    }
    hw->phy.ops.release(hw);
  }
...
static void __e1000e_disable_aspm(struct pci_dev *pdev, u16 state, int locked) {
  ...
  if (parent) {
    pcie_capability_read_word(parent, PCI_EXP_LNKCTL, &parent_aspmc);
  ...
  if (parent)
    pcie_capability_clear_word(parent, PCI_EXP_LNKCTL, aspm_dis_mask);
}
...
static int e1000e_pm_prepare(struct device *dev) {
  return pm_runtime_suspended(dev) && pm_suspend_via_firmware();
}
...
static int e1000_set_features(struct net_device *netdev,
  ...
  if (!(changed &
        (NETIF_F_HW_VLAN_CTAG_RX | NETIF_F_HW_VLAN_CTAG_TX | NETIF_F_RXCSUM |
         NETIF_F_RXHASH | NETIF_F_RXFCS | NETIF_F_RXALL)))
    return 0;
...
static int e1000_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  ...
  netdev->features = (NETIF_F_SG | NETIF_F_HW_VLAN_CTAG_RX |
  ...
  netdev->max_mtu = adapter->max_hw_frame_size - (VLAN_ETH_HLEN + ETH_FCS_LEN);
  ...
  if (adapter->flags & FLAG_APME_IN_WUC) {
    ...
    if ((hw->mac.type > e1000_ich10lan) && (eeprom_data & E1000_WUC_PHY_WAKE))
      adapter->flags2 |= FLAG2_HAS_PHY_WAKEUP;
  } else if (adapter->flags & FLAG_APME_IN_CTRL3) {
    if (adapter->flags & FLAG_APME_CHECK_PORT_B && (adapter->hw.bus.func == 1))
      ret_val = e1000_read_nvm(&adapter->hw, NVM_INIT_CONTROL3_PORT_B, 1,
                               &eeprom_data);
    else
      ret_val = e1000_read_nvm(&adapter->hw, NVM_INIT_CONTROL3_PORT_A, 1,
                               &eeprom_data);
  }
...
static void e1000_remove(struct pci_dev *pdev) {
  ...
  iounmap(adapter->hw.hw_addr);
  if ((adapter->hw.flash_address) && (adapter->hw.mac.type < e1000_pch_spt))
    iounmap(adapter->hw.flash_address);
  pci_release_mem_regions(pdev);
  ...
  if (e1000e_dbg_dir) {
    debugfs_remove_recursive(e1000e_dbg_dir);
    e1000e_dbg_dir = NULL;
  }
}
...
static struct pci_driver e1000_driver = {.name = e1000e_driver_name,
